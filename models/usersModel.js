...
async function getMainAdmin() {
  ...
  if (Number.isInteger(mainAdminId) && mainAdminId > 0) {
    return findById(mainAdminId);
  }

  const mainAdminEmail = process.env.MAIN_ADMIN_EMAIL;
  if (mainAdminEmail) {
    const rows = await db.query("SELECT * FROM users WHERE LOWER(email) = LOWER(?)", [mainAdminEmail]);
    return rows.length ? toUser(rows[0]) : null;
  }
